    ...
    public partial class frm_quan_ly_nhan_vien : Form
    {
        ...
        {    // Lấy giá trị từ các controls
            ...
            try
            {
                ...
                StringBuilder queryBuilder = new StringBuilder("SELECT * FROM staffremake WHERE ");

                bool conditionAdded = false;

                if (!string.IsNullOrEmpty(staffID))
                {
                    queryBuilder.Append($" `Mã nhân viên` = '{staffID}'");
                    conditionAdded = true;
                }
                else
                {
                    if (!string.IsNullOrEmpty(staffName))
                    {
                        ...
                        queryBuilder.Append($" `Tên nhân viên` LIKE '%{staffName}%'");
                        conditionAdded = true;
                    }
                    ...
                    if (!string.IsNullOrEmpty(staffEmail))
                    {
                        if (conditionAdded)
                            queryBuilder.Append(" OR");
                        queryBuilder.Append($" `Email nhân viên` LIKE '%{staffEmail}%'");
                        conditionAdded = true;
                    }
                    ...
                    if (!string.IsNullOrEmpty(staffSex))
                    {
                        if (conditionAdded)
                            queryBuilder.Append(" OR");
                        queryBuilder.Append($" `Giới tính` LIKE '%{staffSex}%'");
                        conditionAdded = true;
                    }
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if ((e.ColumnIndex == dataGridView1.Columns["XoaColumn"].Index) && (e.RowIndex >= 0))
            {
                ...
                int staffID = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells["Mã nhân viên"].Value);
        ...
        private void LoadDataIntoDataGridView()
        {
            try
            {
                ...
                // Thực hiện truy vấn
                string query = "SELECT * FROM staffremake";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            frm_them_nhan_vien formManageStaff_AddnewStaff1 = new frm_them_nhan_vien();
        ...
        { 
                ...
                {
                    connection.Open();
                    string deleteQuery = "DELETE FROM staffremake WHERE `Mã nhân viên` = @Staff_ID";
                    ...
                    string selectAllQuery = "SELECT * FROM staffremake";
        ...
        private bool IsStaffIDExists(string staffID)
        {
            ...
            {
                connection.Open();
                string checkQuery = "SELECT COUNT(*) FROM staffremake WHERE `Mã nhân viên` = @Staff_ID";
